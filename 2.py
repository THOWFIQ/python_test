...
# Flask app (was missing in your snippet)
app = Flask(__name__)
...
# These were referenced but missing in your code; adding them to avoid NameError
@dataclass
...
def mainfunction(filters, format_type, region):
    ...
    # Support typo and correct key for fulfillment ID filter
    ...
    if "foid" in filters:
        ...
        for batch in chunk_list(fo_ids, 50):
            payload = {"query": fetch_getByFoid_query(json.dumps(batch))}
            response = requests.post(url, json=payload, verify=False)
            data = response.json()
            if "errors" in data:
                return jsonify({"error": data["errors"]}), 200
    ...
    if "wo_id" in filters:
        ...
        for batch in chunk_list(wo_ids, 50):
            payload = {"query": fetch_getByWoId_query(json.dumps(batch))}
            response = requests.post(url, json=payload, verify=False)
            data = response.json()
            if "errors" in data:
                return jsonify({"error": data["errors"]}), 200
    ...
    for obj in records:
        ...
        if getattr(obj, "vendorInfo", None):
            graphql_request.append({
                "url": path['VENDOR'],
...
def OutputFormat(result_map, format_type=None, region=None):
    try:
        ...
        def pick_address_by_type(salesheader_entry: Dict, contact_type: str) -> Dict:
            """
            From getSoheaderBySoids entry, pick the address where any contact.contactType == contact_type
            """
        ...
        def get_install_instruction2_id(fulfillment_entry: Dict) -> str:
            """
            From getFulfillmentsById entry, scan all salesOrderLines[].specialinstructions[]
            and return specialInstructionId where specialInstructionType == 'INSTALL_INSTR2'
            """
        ...
        for idx in range(N):
            ...
            row = {
                ...
                # ----- Original fields you already had -----
                ...
                # NOTE: keep original "Shipping Country" from first address if needed
                ...
                # Removed old "Company Name", "Contact Type", "City", "First Name", etc.
                # They are replaced by the new shipping/billing fields above.
        ...
        if len(flat_list) > 0:
            if format_type == "export":
                ...
                # Keep your existing table builder but update the desired order to include new fields
                desired_order = [
                    "BUID",
                    # New fields
                    ...
                    # Removed old company/contact/city/name fields replaced by the new ones above
...
def safe_get(data, path, default=""):
    """
    Safe deep getter that mirrors your original behavior:
    - Walk through dictionaries/lists via keys/indices in `path`
    - Returns str(value) if found, else default (empty string)
    """
    try:
        for key in path:
