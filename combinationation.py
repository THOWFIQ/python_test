...
PRIMARY_FIELDS = {
    "Sales_Order_id", "wo_id", "Fullfillment Id", "foid", "order_date", "Order_from_date", "Order_from_date"
}
SECONDARY_FIELDS = {
...
def combined_OrderDate_fetch(orderFromDate, orderToDate, region, filters):
    try:
        ...
        # print(f"[DEBUG] orderDate_query: {orderDate_query}")

        orderDate_data = post_api(URL=path['SOPATH'], query=wo_query, variables=None)
        # print(f"[DEBUG] orderDate_query API Response: {wo_data}")

        if orderDate_data and orderDate_data.get('data'):
            combined_orderDate_data['data']['getByWorkorderids'] = orderDate_data['data'].get('getByWorkorderids', {})
        print(json.dumps(combined_orderDate_data,indent=2))
        exit()
...
if __name__ == "__main__":
    ...
    sample_filters ={
            ...
            Order_from_date :"2025-01-01T14:00:00",
            Order_to_date :"2025-06-01T14:37:01",
            # "ISMULTIPACK": "Yes",
            # "BUID": "202",
    ...
    flat_table = OutputFormat(output,format_type)
