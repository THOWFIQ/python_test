...
def getbyOrderDateRange(orderFromDate, orderToDate, format_type, region):
    ...
    sales_order_chunks = list(chunked_iterable(records, 5))
    ...
    def process_chunk(chunk):
        ...
        for record in chunk:
            ...
            try:
                result = getbySalesOrderIDs(salesorderid, format_type)
                if isinstance(result, list):
                    result_chunk.extend(result)
            except Exception as e:
                print(f"[ERROR] salesOrderId: {salesorderid} failed: {e}")
        return result_chunk

    with ThreadPoolExecutor(max_workers=5) as executor:
        futures = [executor.submit(process_chunk, chunk) for chunk in sales_order_chunks]
