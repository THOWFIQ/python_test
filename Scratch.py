...
def fileldValidation(filters, format_type, region):
    ...
    if 'Sales_Order_id' in primary_filters:
        ...
        result_map['Sales_Order_id'] = threaded_fetch(fetch_soaorder_query, "salesorderIds", so_ids, SOPATH)

    if 'foid' in primary_filters:
        foids = list(set(x.strip() for x in primary_filters['foid'].split(',') if x.strip()))
        result_map['foid'] = threaded_fetch(fetch_fm_order_query, "foids", foids, FOID)

    if 'wo_id' in primary_filters:
        woids = list(set(x.strip() for x in primary_filters['wo_id'].split(',') if x.strip()))
        result_map['wo_id'] = threaded_fetch(fetch_wo_details_query, "woIds", woids, WOID)
    ...
    if 'order_date' in primary_filters:
        ...
        result_map['order_date'] = threaded_fetch(fetch_order_date_query, "orderDates", dates, SOPATH)
