def OutputFormat(result_map, format_type=None, secondary_filters=None):
    ...
    for so_index, so_entry in enumerate(sales_orders):
        try:
            ...
            # Ensure both headers and orders are lists
            get_soheaders = so_data.get("getSoheaderBySoids", [])
            if isinstance(get_soheaders, dict):
                get_soheaders = [get_soheaders]
            elif not isinstance(get_soheaders, list):
                get_soheaders = []

            get_salesorders = so_data.get("getBySalesorderids", [])
            if isinstance(get_salesorders, dict):
                get_salesorders = [get_salesorders]
            elif not isinstance(get_salesorders, list):
                get_salesorders = []
