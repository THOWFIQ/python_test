...
def mainfunction(filters, format_type, region):
                ...
                if ship_from_vendor_id and source_manifest_id:
                    ...
                    try:
                        if asn_response.status_code == 200:
                            ...
                            print(f"ASN API returned status code {asn_response.status_code}")
                            print("Response text:", asn_response.text)
                    except Exception as e:
                        print("Error parsing ASN response JSON:", e)
                        print("Raw response:", asn_response.text)
                    ...
                    record.vendorsiteid = None
                    if isinstance(asn_header, list) and asn_header and isinstance(asn_header[0], dict):
                        record.vendorsiteid = asn_header[0].get("shipToVendorSiteId")
                    else:
    ...
    for obj in records:
        ...
        if hasattr(obj, "vendorsiteid") and obj.vendorsiteid:
            graphql_request.append({
                "url": path['VENDOR'],
                "query": fetch_isCFI_query(json.dumps(obj.vendorsiteid))
            })
            
            print(f"[{countReqNo}] VendorTo ID: {fulfillment_id}")
        
        if hasattr(obj, "workOrders") and obj.workOrders:            
            if isinstance(obj.workOrders, list) and obj.workOrders:
                first_workid = obj.workOrders[0]
            else:
                first_workid = obj.workOrders
...
def OutputFormat(result_map, format_type=None, region=None):
    try:
        ...
        print(json.dumps(salesorders,indent=2))
        exit()
