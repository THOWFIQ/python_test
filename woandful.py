...
def getbyFulfillmentIDs(fulfillmentid, format_type,workorderidlist):
    ...
    for iworkorderID in workorderidlist:
        
        workOrderId_query = fetch_workOrderId_query(iworkorderID)

        getWorkOrderById=post_api(URL=WOID, query=workOrderId_query, variables=None)
       
        getByWorkorderids_query = fetch_getByWorkorderids_query(iworkorderID)

        sn_numbers = []
        getByWorkorderids=post_api(URL=FID, query=getByWorkorderids_query, variables=None)
        # print(f"result of : {getByWorkorderids}")
...
def getFullfillmentIDbyWorkorderID(workorderids, format_type, region,workorderidlist):

    getByWorkorderids_query = fetch_getByWorkorderids_query(workorderids)
       
    getByWorkorder_result = post_api(URL=FID, query=getByWorkorderids_query, variables=None)

    fulfillmentIds = getByWorkorder_result["data"]["getByWorkorderids"]["result"][0]["fulfillment"]["fulfillmentId"]

    final_result = getbyFulfillmentIDs(fulfillmentid=fulfillmentIds,format_type=format_type,workorderidlist=workorderidlist)
  
    return final_result

def workerORderThread(workorderids, format_type, region):
    total_output = []

    if not workorderids:
        return {"error": "Workorderids is required"}
    ...
    def fetch_workorderid(wid):
        try:
            return getFullfillmentIDbyWorkorderID(wid, format_type, region,workorderids)
        except Exception as e:
            print(f"Error fetching {wid}: {e}")
            return []

    with ThreadPoolExecutor(max_workers=5) as executor:
        futures = [executor.submit(fetch_workorderid, wid) for wid in workorderids]
...
#     workorderids=["7360928459","7360928459"]
#     region = "EMEA"
#     # fulfillmentIds=["262135", "271698"]
#     # cleaned = fetch_and_clean()
#     # print(cleaned)
#     format_type='export' #grid/export
#     workerORderThread(workorderids=workorderids,format_type=format_type,region=region)
