...
def newmainfunction(filters, format_type, region):
    ...
    if "Sales_Order_id" in filters:
        ...
        if filters.get(salesOrder_key):
            ...
            # print(salesorder_ids)
            # exit()
            for soid in chunk_list(salesorder_ids,10):
                # graphql_request.append({
                #         "url": path['FID'],
                #         "query": fetch_keysphereSalesorder_query(soid)
                #     })
    ...
    if "wo_id" in filters:
        ...
        workOrder_key = "wo_id"
        matched = False
        uniqueSalesOrder_ids = ",".join(sorted(set(filters[workOrder_key].split(','))))
        filters[workOrder_key] = uniqueSalesOrder_ids

        if filters.get(workOrder_key):
            workorder_ids = list(map(str.strip, filters[workOrder_key].split(",")))
            for woid in chunk_list(workorder_ids,10):
                graphql_request.append({
                        "url": path['WORKORDER'],
                        "query": fetch_workOrder_query(woid)
                    })
                # print(f"Sequence of SALES ORDER DATA : {soid_chunk}")
